...
static void update() {
    ...
    FOR_EACH(walls, i) {
        ...
        if (fa < ta) {
            float fx = normalized_sinf(fa) * radius * (w->isFloating ? 1.2f : 1.1f);
            float tx = normalized_sinf(ta) * radius * (w->isFloating ? 1.2f : 1.1f);
    ...
    if (a > -M_PI / 2.0f && a < M_PI / 2.0f) {
        rect(50.0f + normalized_sinf(a) * radius, ho, 1.0f, -100.0f);
    }
    a = wrap(player.angle + M_PI / 2.0f, -M_PI, M_PI);
    if (a > -M_PI / 2.0f && a < M_PI / 2.0f) {
        rect(50.0f + normalized_sinf(a) * radius, fmod(ho + 100.0f, 400.0f), 1.0f, -100.0f);
    }
    a = wrap(player.angle + M_PI, -M_PI, M_PI);
    if (a > -M_PI / 2.0f && a < M_PI / 2.0f) {
        rect(50.0f + normalized_sinf(a) * radius, fmod(ho + 200.0f, 400.0f), 1.0f, -100.0f);
    }
    a = wrap(player.angle + (M_PI / 2.0f) * 3.0f, -M_PI, M_PI);
    if (a > -M_PI / 2.0f && a < M_PI / 2.0f) {
        rect(50.0f + normalized_sinf(a) * radius, fmod(ho + 300.0f, 400.0f), 1.0f, -100.0f);
    }
    ...
    FOR_EACH(walls, i) {
        ...
        if (w->isFloating) {
            ...
            if (fa < ta) {
                float fx = normalized_sinf(fa) * radius * 1.2f;
                float tx = normalized_sinf(ta) * radius * 1.2f;
